    ...
    class API extends REST {
        ...
        */
        private function dbConnect(){
           // $this->db = mysqli_connect( self::DB_SERVER,self::DB_USER, self::DB_PASSWORD);
            ...
            $this->db = new mysqli('localhost','root','Kr23JjC5js','webandtv');
        }
        ...
        private function sw(){
            ...
            $website_type = $this->_request['website_type'];
            if($website_type==1){
                $website_linkname_parent = $this->_request['website_linkname_parent'];
            }
            else {
                $website_linkname_parent = '';
            }
            $website_description = '';
            $website_keywords = '';
            $website_icon = '';
            $website_date = gmdate('Y/m/d H:i:s');
            $website_update = gmdate('Y/m/d H:i:s');
            $website_status = 1;
            ...
            if ($this->verifyUserToken($user_id,$user_token)) {
                ...
                $sql = "INSERT INTO `website` (user_id,website_name,website_linkname,website_type,website_linkname_parent,website_description, website_keywords, website_icon, website_date, website_update,website_status) VALUES ('$user_id','$website_name','$website_linkname','$website_type','$website_linkname_parent','$website_description', '$website_keywords', '$website_icon', '$website_date', '$website_update','$website_status')";
                $mysql->query($sql);
                ...
                if ($mysql->affected_rows==1) {
                    ...
                    if($website_type==1){
                        $row_parent = $this->getPrivateDataWebsiteLinkname($website_linkname_parent);
                        $website_parent_id = $row_parent['website_id'];
                        $module_name = $website_name;
                        $module_link = 'https://weband.tv/'.$website_linkname;
                        $module_dashboard = '';
                        $mysql=$this->db;
                        $sql = "INSERT INTO module (website_id,module_name,module_link,module_dashboard) VALUES ('$website_parent_id','$module_name','$module_link','$module_dashboard')";
                        $mysql->query($sql);
                        $data_pre['website_parent_id']=$website_parent_id;
                    }
        ...
        private function gw(){
            ...
            if ($website_linkname) {
                ...
                $data = array();
                if ($website_linkname){
                    $results = $mysql->query("SELECT * FROM website LEFT JOIN social ON website.website_id = social.website_id LEFT JOIN link ON website.website_id = link.website_id WHERE website_linkname = '$website_linkname' LIMIT 1");
                }
        ...
        */
        ...
        private function getModule(){
            ...
            if ($this->verifyUserToken($user_id,$user_token)) {
                $mysql=$this->db;
                $results = $mysql->query("SELECT * FROM module WHERE module_id = '$module_id' LIMIT 1");
        ...
        private function addToProgram(){
            ...
            if($this->verifyUserToken($user_id,$user_token)){
                ...
                if($mysql->affected_rows==1){
                    ...
                    $data_pre['errrorrrr']= $mysql->error;
                    $data_pre['user_id']=$user_id;
                    $data_pre['user_token']=$user_token;
                    $data_pre['website_id']=$website_id;
                    $data_pre['module_id']=$module_id;
                    $data_pre['program_begin']=$program_begin;
                    $data_pre['program_end']=$program_end;
                    $data_pre['program_day']=$program_day;
                    $data_pre['result']='false';
                }
        ...
        private function deleteModule(){
            ...
            if($this->verifyUserToken($user_id,$user_token)){
                $mysql=$this->db;
                $data_pre = array();
                if($mysql->query("DELETE FROM program WHERE module_id='$module_id'")){
                    $mysql->query("DELETE FROM module WHERE module_id='$module_id' LIMIT 1");
                    $data_pre['result']='true';
                }
                else {
                    $data_pre['result']='false';
        ...
        private function getPlaylist(){
            ...
            if ($website_linkname) {
                ...
                //$data = mysqli_fetch_assoc($results);
                $row = mysqli_fetch_assoc($results);
                $data_result['website_id'] = $row['website_id'];
                $data_result['website_linkname_parent'] = $row['website_linkname_parent'];
                $data_result['website_type'] = $row['website_type'];
                $data_result['playlist_src'] = json_decode($row['playlist_src']);
        ...
        private function s(){
            ...
            if ($q) {
                $mysql=$this->db;
                $results = $mysql->query("SELECT website_linkname,website_name FROM website WHERE website_name LIKE '%$q%' AND website_status > 0 LIMIT 5");
                $result = 'false';
                while($row = $results->fetch_assoc()) {
                    $data[]=$row;
                    $result = 'true';
